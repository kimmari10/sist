...
<section id="content" class="notice">
	...
	<form class="bbs-search-form" action="" method="post">
		<fieldset>
			...
			<label class="hidden" for="f">분류선택</label> <select name="f">
				<option value="TITLE">제목</option>
				<option value="WRITER">작성자</option>
			</select> <label class="hidden" for="q">검색어</label> <input type="text"
				name="q" value="ok good fine" /> <input type="submit" value="검색" />
		</fieldset>
	</form>
	<h3 class="hidden">공지사항목록</h3>

	<form action="delListProc.htm" method="post">
			...
			<tbody>
				<c:forEach var="n" items="${list}">
					<tr>
						...
						<td class="seq"><input type="checkbox" name="seq"
							value="${n.seq}" />${n.seq}</td>
				...
				<tr>
						<td colspan="5"><input type="submit" value="삭제" />
					</tr>
			</tbody>
		</table>
	</form>
	...
	<p class="bbs-status">
		<span class="strong">1</span> / 2 pages
	</p>

	<p class="bbs-buttons">
		<a class="button btn-reg" href="noticeReg.htm">글쓰기</a>
	</p>

	<div class="bbs-pager">
		<p>
			<img alt="이전" src="../images/btnPrev.png" />
		</p>
		<ul>
